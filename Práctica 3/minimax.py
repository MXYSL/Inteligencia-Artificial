...
class Game:
    ...
    def draw_tree(self, tree):
        ...
        if len(tree) > MAX_NODES:
            self.canvas.create_text(canvas_w//2, canvas_h - 14,
                text=f"… mostrando {MAX_NODES}/{len(tree)} nodos",
    ...
    def reset_game(self):
        ...
        self.canvas.create_text(240, 240, text="Juega para ver el árbol",
                                fill=TEXT, font=self.font_label)
        self.nodes_label.config(text="Nodos explorados: —")
        self._set_status("TU TURNO  [ O ]", TEXT)
        for i in range(SIZE):
            for j in range(SIZE):
                self.buttons[i][j].config(text=" ", bg=C_FONDO,
                                           fg=TEXT, state="normal",
                                           cursor="hand2")
# ─────────────────────────────────────────────
# MAIN
# ─────────────────────────────────────────────
